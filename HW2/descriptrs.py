...
class Port:
    def __set__(self, instance, value):
        ...
        if not 1023 < value < 65536:
            a = 'клиента' if str(type(instance)) == "<class '__main__.Client'>" else 'сервера'
            instance.LOGGER.critical(
                f'Попытка запуска {a} с указанием неподходящего порта {value}. Допустимы адреса с 1024 до 65535.')
            exit(1)
